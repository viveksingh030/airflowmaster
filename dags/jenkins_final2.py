...
def trigger_jenkins_job(job_name, deployedEnv, team,jenkins_conn_id='jenkins_http', **kwargs):
    """Trigger a Jenkins job and return the build number."""
    conn = get_jenkins_connection(jenkins_conn_id)
    url = f"{conn['host']}:{conn['port']}/job/{job_name}/buildWithParameters?delay=0sec&deployedEnv={deployedEnv}&team={team}"
...
def monitor_jenkins_job(job_name, deployedEnv, team, build_number, jenkins_conn_id='jenkins_http', max_retries=40, wait_time=15, **kwargs):
...
# Define the DAG
with DAG(
    'ml_endpoint_deployment_pipeline_final_2',
    ...
    deploy_endpoint = PythonOperator(
        ...
        op_kwargs={'job_name': 'choice-pipeline','deployedEnv':'PROD','team':'Kyx'},
    )
