...
dag = DAG(
    'jenkins_http_pipeline_2',
    default_args=default_args,
    description='Trigger Jenkins build and wait for completion',
    schedule=None,
    catchup=False
)
...
wait_for_jenkins = HttpSensor(
    task_id='wait_for_jenkins_completion',
    http_conn_id=jenkins_conn_id,
    endpoint='/job/envPipeline/lastBuild/api/json',
    method='GET',
    headers={
        "Authorization": f"Basic {encoded_auth}"  # Use encoded username:password here
    },
    response_check=lambda response: response.json().get('result') == 'SUCCESS',
    poke_interval=20,  # Check every 30 seconds
...
trigger_jenkins >> wait_for_jenkins >> print_hello_task
